...
lx=0.58; ly=0.22; lz=0.24; %room dimension in (m)
h=lz; %distance between source and reciver plane (m)
[XT,YT]= meshgrid(0,0); %position of LED

Nx = lx*100; Ny= ly*100; %number of grids in the receiver plane
x=-lx/2:lx/Nx:lx/2;
y=-ly/2:ly/Ny:ly/2;
[XR, YR] = meshgrid(x, y);
...
P_rx_total =(P_rx);
%P_rx_initial =P_rx;
%P_rx_final =P_rx - (2.01-12.8.*D+19.9.*(D.^(2)));
%P_rx_final =P_rx - (-4.61+57.2.*D-225.*(D.^(2))+283.*(D.^(3)));
%P_rx_final =P_rx - (58.6-736.*D+3538.*(D.^(2))-7654.*(D.^(3))+6255.*(D.^(4)));
%P_rx_final =P_rx - (-4540+93611.*D-801263.*(D.^(2))+3.64.*(10.^(6)).*(D.^(3))-9.29.*(10.^(6)).*(D.^(4))+1.26.*(10.^(7)).*(D.^(5))-7.08.*(10.^(6)).*(D.^(6)));
