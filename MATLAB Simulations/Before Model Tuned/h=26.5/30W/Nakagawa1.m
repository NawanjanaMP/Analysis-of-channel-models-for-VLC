...
P_LED=5250;%transmitted optical power by individual LED
%nLED=60;
...
% P_rx_initial =P_rx;
% %P_rx_final =P_rx + (1031-14532.*D+76794.*(D.^(2))-180212.*(D.^(3))+158467.*(D.^(4)));
% %P_rx_final =P_rx - (18-192.*D+683.*(D.^(2))-809.*(D.^(3)));
% %P_rx_final =P_rx - (-0.272+2.18.*D-3.42.*(D.^(2)));
% P_rx_final =P_rx - (0.249.*D+1.33.*(10.^(-3)));
